...
5. Have Glen run the [promoter-cofactor tutorial](tutorial/08-14_promoter_cofactor_browser.md) against the exact candidate: live stale-form/copy binding, saved evidence, verified plus/minus feature preview/confirmation/undo, then the optional IRF9 package. Human scientific approval remains pending; unstranded evidence remains regions and raw coverage reconstruction is not supported by the reduced package.
   Validate DuckDB boundaries/limits, scan filters and selected-package GUI staleness; have Glen inspect [imported-hit triangles](tss_tfbs_profiles.md#imported-duckdb-motif-hits) on both strands and run the [regulatory/TSS-subset reader](regulatory_motif_subset.md) against the actual genome-wide delivery (including MT and source floors); synthetic fixture acceptance is not production acceptance. Native TSS follow-up (not an added release gate): live acceptance of [report attachment](gui.md#attach-the-quantitative-report) and [view SVG export](gui.md#export-the-native-tss-view), including CD44/TGFB1 gaps, axes, selected lanes and sparse-hit coverage; reporter rows remain open. Validate approved [local scoring](gui.md#compute-local-tss-scores): InlineSequence/matrix provenance, separate score kinds/scales/cache, cancellation, stale-form handling and genuine zero versus unavailable masks. New live database queries still require exact reference-bound interval admission; retain [flat-file limits and readability plan](tss_annotated_sequence_readability_plan.md). Glen: regenerate [source-coherent Ensembl/RefSeq pages](transcript_source_presentation.md) from one enriched locus and verify mappings/PDF/SVG/HTML receipts; synthetic coverage is not real-data acceptance. Keep the [standalone comparison](transcript_source_presentation.md#standalone-comparison-review) deferred rather than adding a second source-join implementation.
...
## .12 Priorities
Selected aim: responsive startup and DNA-feature presentation, separately from `.11` acceptance. The [reconciled latency plan](dna_feature_rendering_latency_plan.md) owns the work sequence and DEC-039 audit gates. The owner selected linear loci through 250 kbp / 5,000 loaded features as the interactive envelope on 2026-09-24; 2 Mbp and 10,000-feature cases remain stress tests. Circular correctness and native interaction regressions remain required, with no `.12` circular timing budget. These scope decisions are not accepted performance.
Next: Glen's prebuilt baseline must include the exact 250 kbp / 5,000-feature case and derived-layer inventories, not a legacy nine-fixture receipt. Retain native attribution, a local macOS cross-check and the opt-in [startup CPU checkpoints](../benches/README.md#startup-phase-checkpoints), which do not prove native presentation. Separately, the [exact-revision handoff](glen_gui_latency_handoff_20260924.md) compares pan/GC-count work at `ad0338a7` and `ee9eb483` under identical effective profiles; arithmetic-count correctness is not a responsiveness verdict or current boundary coverage. Follow the [measurement-profile contract](dna_feature_rendering_latency_plan.md#measurement-profiles): packaged-release native acceptance, `bench-audit` ladder continuity, and Glen's separate repeated same-SHA PATZ1 native profile comparison, not a universal timing correction or installer-CI benchmark stage. S6 owns startup/first-content fixes and may lead; remaining S1-S5 work follows measured runtime contribution, while B0 handles build feedback independently without authorizing a broad crate split. Preserve scientific output, freshness and cancellation; retain target-bound acceptance separately from stress tests and harness timing. Recheck the selected-assay cDNA overview/detail at the merged SHA, retaining Glen's `18ae846e` visual review separately; genomic projection remains deferred pending a bound cDNA/exon mapping contract.
...
- Cryptic-splicing v2 follow-up: consider explicitly versioned segmented spans
  for origin-crossing circular sequences, non-GT/AG boundary policies, and an
  opt-in typed ORF-inference contract. Do not fold those inferences into the v1
  annotation-bound CDS or evidence-layer semantics.
